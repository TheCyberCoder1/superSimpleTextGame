...
user = 0
while user != 2:
    print("Which floor would you like to look in? 1, 2 or 3?")
    user = int(getch.getch())
    if user == 1 or user == 3:
...
while user != 2:
    print("Which area would you like to look in: living room (1), office (2), bedroom area (3)?")
    user = int(getch.getch())
    if user == 1 or user == 2:
        print("hmmm.... no sign of any mad CEO")
        continue
